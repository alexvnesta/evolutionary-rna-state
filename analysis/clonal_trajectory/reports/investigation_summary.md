...
Closing the loop left open by the deconvolution negative: using malignant-compartment-specific genes (rather than composition correction) on the 53-sample DDLPS matched bulk. De-novo clone labels across 29 patients gave a per-gene malignant-vs-reference specificity score (208 genes above 0.5; the top-specific genes are the expected DDLPS mesenchymal/ECM program COL1A1/COL3A1/DCN/LUM/SPARC). Supervised leave-one-out on the 19 labeled matched-bulk patients: naive top-3000-variance features give balanced accuracy 0.55 (AUC 0.56, p=0.30, the Track 1 baseline); the 203 malignant-specific genes lift it to 0.64 (AUC 0.67, p=0.17); the 49 strongly-specific genes reach AUC 0.73. Compartment restriction improves real-bulk transfer in point estimate, reproducing the controlled-cSCC direction on real matched bulk, but it is not significant at n=19, exactly the position the power curve predicts (a moderate effect needs N about 40). The mechanism is confirmed on three independent settings; the sample size is the gate. Detail in ddlps_compartment_bulk_report.md.

...
### Compartment-specific features on real DDLPS matched bulk — mechanism confirmed, n-gated

Closing the loop left open by the deconvolution negative: using malignant-compartment-specific genes (rather than composition correction) on the 53-sample DDLPS matched bulk. De-novo clone labels across 29 patients gave a per-gene malignant-vs-reference specificity score (208 genes above 0.5, of which 203 are measured in the bulk matrix and used as features; the top-specific genes are the expected DDLPS mesenchymal/ECM program COL1A1/COL3A1/DCN/LUM/SPARC). Supervised leave-one-out on the 19 labeled matched-bulk patients: naive top-3000-variance features give balanced accuracy 0.55 (AUC 0.56, p=0.30, the Track 1 baseline); the 203 malignant-specific bulk-measured genes lift it to 0.64 (AUC 0.67, p=0.17); the 49 strongly-specific genes reach AUC 0.73. Compartment restriction improves real-bulk transfer in point estimate, reproducing the controlled-cSCC direction on real matched bulk, but it is not significant at n=19, exactly the position the power curve predicts (a moderate effect needs N about 40). The mechanism is confirmed on three independent settings; the sample size is the gate. Detail in ddlps_compartment_bulk_report.md.

### LUAD (Kim GSE131907) — fourth-tissue replication and a boundary condition

A GEO survey (cohort_survey.md) confirmed that matched scRNA+real-bulk from the same 40-plus patients is essentially absent from public repositories, but identified the Kim lung adenocarcinoma atlas as the largest single-tissue scRNA cohort with author annotation. Restricting to tumor-derived samples with adequate epithelial content gave 26 tumors, the largest single cohort in the project and a fourth (glandular-epithelial) tumor lineage.
- The clone-imprint replicates a fourth time: 25 of 26 tumors show CNV ratio at or above 1.5x, and 15 of 16 multi-clone tumors beat the independent-expression permutation null (median z 9.9, up to 134); within-patient 0.78 versus between-patient 1.05 SD/gene, matching the other cohorts.
- A boundary condition emerged: LUAD's interferon/MHC axis is continuous, not the discrete hot/cold split of cSCC/DDLPS/AML. Its oncogenic programs do not anti-correlate with immune signaling the way the DDLPS MDM2 amplicon did.

Adding LUAD to the pool (N=145) keeps the grouped-CV result strongly significant but lowers it to balanced accuracy 0.79 (AUC 0.87, p=0.005) from the three-cohort 0.91, because LUAD's graded axis forced into a median-split label adds noise. The lesson is that past the power floor the binding constraint becomes how discretely a tissue's immune axis is structured, not sample count. Leave-one-tissue-out improved for AML (0.53 to 0.74) with more training tissues but stayed within-tissue-dominated for the rest. Detail in luad_report.md and pooled4_classifier_report.md.

### ICB outcome test on Riaz melanoma bulk (GSE91061) — coherent negative

The first test against real checkpoint-response outcomes. Of the three classic melanoma bulk-ICB cohorts, only Riaz is adequately powered (49 pre-treatment samples, 10 responders / 39 non-responders; Auslander has 2 responders, Hugo n=28). Because Riaz has no matched single-cell data, the clonal trajectory class cannot be inferred there; what is testable is whether the malignant-intrinsic interferon/MHC signature, scored in pre-treatment bulk, associates with response and adds anything beyond bulk immune infiltration.
- Neither signature predicts response at this sample size: malignant-intrinsic IFN/MHC AUC 0.56 (p=0.56), immune-infiltration proxy AUC 0.62 (p=0.24).
- The two signatures correlate at r=0.84 in bulk, the same Simpson's-paradox confound quantified in DDLPS. In a joint model neither term is significant and IFN/MHC adds nothing beyond infiltration.

This is a coherent negative, not a contradiction: it confirms at the ICB endpoint what the expression-level work showed, that reading the malignant-intrinsic clonal immune-state from bulk needs compartment resolution bulk does not provide, and naive bulk scoring recovers only the (underpowered) infiltration signal. The blocker is the bulk-transfer step, not the biology. Detail in riaz_icb_report.md.

## The honest synthesis

Four things are now established with real data. First, the core biological premise holds and generalizes: clone-defined states imprint the transcriptome genome-wide, form recurrent cross-patient classes, and (in three of four cancers) those classes couple to antigen presentation, across squamous cSCC, mesenchymal DDLPS, hematopoietic AML, and glandular LUAD. Second, the supervised label-recovery method works once it is properly powered: at pooled N=99 the harmonized immune-state label is recovered at balanced accuracy 0.91 (p=0.005), directly confirming that the earlier n=19 failure was sample size, not concept; adding a fourth tumor with a graded immune axis (LUAD, N=145) keeps this significant at 0.79, showing the achievable ceiling depends on how discretely a tissue's immune axis is structured. Third, two hard limits are now quantified rather than assumed: reading the malignant-intrinsic state from real bulk is not rescued by composition-based deconvolution (the axis lives within the malignant compartment, not in mixing proportions), and the trained classifier is within-tissue, not zero-shot pan-cancer.
...
1. Reach N about 40 matched same-tissue scRNA+bulk to certify the compartment-specific remedy. It now shows a positive point estimate on real DDLPS bulk (balanced accuracy 0.55 to 0.64) but is not significant at n=19; the mechanism is validated across three settings and the only remaining gate is sample size.
...
4. The bulk ICB outcome test has now been attempted on Riaz (the only adequately-powered cohort) and returns a coherent negative: naive bulk signature scoring recovers only immune infiltration, so a matched single-cell reference or a compartment-specific feature set certified at N about 40 is required before the ICB association can be fairly evaluated. This is the decisive remaining experiment and it is gated on cohort availability.

## Artifact index

Reviews and reports: clonal_evolution_bulkRNA_ICB_review.md, phase0_report.md, ddlps_report.md, aml_report.md, ddlps_deconvolution_report.md, ddlps_compartment_bulk_report.md, luad_report.md, riaz_icb_report.md, pooled_classifier_report.md, pooled4_classifier_report.md, cohort_survey.md, clonal_evo_feasibility_protocol.md.
Figures: phase0_summary.png, phase0_validation.png, phase0_classes.png, phase0_dilution.png, phase0_compartment.png, ddlps_replication.png, ddlps_bulk_transfer.png, ddlps_deconvolution.png, ddlps_compartment_bulk.png, aml_replication.png, luad_replication.png, riaz_icb_test.png, pooled_classifier.png, pooled4_classifier.png, clonal_evo_power.png, clonal_evo_data_flow.png.
Data: clonal_evo_data_manifest.csv, clonal_evo_power_grid.csv, clonal_evo_matched_counts.csv, cscc_infercnv_summary.csv, cscc_tumor_clone_assignments.csv, phase0_validation.csv, clone_programs_classified.csv, phase0_dilution.csv, ddlps_infercnv_summary.csv, ddlps_validation.csv, ddlps_clone_programs_classified.csv, ddlps_bulk_scores.csv, ddlps_loo_preds.csv, ddlps_deconv_fractions.csv, ddlps_deconv_correlations.csv, aml_infercnv_summary.csv, aml_validation.csv, aml_clone_programs_classified.csv, pooled_results.json, pooled_labels_preds.csv, pooled4_results.json, luad_infercnv_summary.csv, luad_validation.csv, ddlps_compartment_specificity.csv.

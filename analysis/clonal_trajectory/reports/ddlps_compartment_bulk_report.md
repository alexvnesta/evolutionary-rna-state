...
labeled each cell as malignant-clone or diploid-reference. Pooling those labels
genome-wide gave a per-gene compartment-specificity score (mean malignant log-norm minus
mean reference log-norm); 203 genes scored above 0.5 and 49 above 1.0.
The top-specific genes are the expected DDLPS mesenchymal/ECM program (COL1A1, COL3A1,
DCN, LUM, SPARC). A supervised classifier (L2 logistic, leave-one-out on the 19 labeled
matched-bulk patients, target = per-patient dominant clone class) was trained on three
feature sets: naive top-3000-variance (the Track 1 baseline), the 203
malignant-specific genes, and the 49 strongly-specific genes. Significance by 1000-fold
label permutation.
...
| features | n genes | balanced accuracy | AUC | perm p |
|---|---|---|---|---|
| naive top-3000 variance | 3000 | 0.55 | 0.56 | 0.30 |
| malignant-specific (spec>0.5) | 203 | 0.64 | 0.67 | 0.17 |
| strongly specific (spec>1.0) | 49 | 0.60 | 0.73 | — |

Balanced accuracy rises from 0.55 (naive, the Track 1 result) to
0.64 with compartment restriction, and the strongly-specific 49-gene set
reaches AUC 0.73 (better sample ranking) though a slightly lower balanced
accuracy at the fixed threshold. The improvement direction validated in controlled cSCC
data reproduces on real matched bulk.

...
Context (see docs/PROJECT_STATUS.md):
  ...
  - This script joins the 66-feature non-reference matrix (40 existing + 66 new)
    to the pre-assembled immune-floor covariates (phase2_covariates_n106.parquet,
    keyed on run_accession) and runs the pre-registered floor-vs-nonref eval.
...
def load_existing_matrix(existing_matrix: Path) -> pd.DataFrame:
    """The 40-sample (n<=25 committed) non-ref feature matrix, keyed on run_accession."""
...
def two_block_eval(mat: pd.DataFrame, cov: pd.DataFrame,
    ...
    floor-conditioned (C = A+B). Grouped 5-fold within-cohort + LOCO, permutation p.
    Faithful to the eval session's harness: 20-seed-averaged 5-fold CV AUROC,
    fold-contained residualization, cohort-internal permutation null.
    """
    ...
    feat_cols = [c for c in mat.columns]
    X_nonref = df[feat_cols].astype(float).fillna(df[feat_cols].astype(float).median())
    X_floor  = df[floor_cols].astype(float).fillna(0.0)

    def cv_auroc(X, seeds):
        aucs = []
        for s in seeds:
    ...
    out = {
        ...
        "floor_20seed": cv_auroc(X_floor, seeds),
        "nonref_20seed": cv_auroc(X_nonref, seeds),
        "floor_plus_nonref_20seed": cv_auroc(
            pd.concat([X_floor.reset_index(drop=True),
                       X_nonref.reset_index(drop=True)], axis=1), seeds),
    }
    out["delta_C_minus_A"] = round(out["floor_plus_nonref_20seed"] - out["floor_20seed"], 4)
    return out
